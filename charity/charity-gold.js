...
function ensureShell() {
  ...
  app.innerHTML = `
    <header id="header-slot" class="sticky top-0 z-30 bg-panel/90 backdrop-blur-xl"></header>
...
function renderHeader() {
  ...
  el.innerHTML = `
    <div class="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
      <div class="w-28">${['batch','dist','cardDetail','qr','proof'].includes(state.viewMode) ? `<button data-action="back" class="btn-action-flat btn-action-sm inline-flex items-center gap-2 text-sm font-bold">${icon('arrow-left','w-4 h-4')} Back</button>` : ''}</div>
      <h1 class="text-center text-sm font-extrabold uppercase tracking-[0.18em] text-gold">${getHeaderTitle()}</h1>
      <div class="w-28"></div>
    </div>`;
}
...
function buildVoucherList() {
  const rows = (state.filterStatus === 'ALL' ? mockBatches : mockBatches.filter(b => b.status === state.filterStatus)).map(batch => `<article class="rounded-2xl border border-line bg-panel p-5 shadow-luxury transition hover:-translate-y-1 hover:border-gold/60"><div class="flex items-start justify-between gap-3"><span class="text-[13px] font-semibold tracking-wide text-cream sm:text-[15px]">Batch #${batch.id}</span><div class="text-right"><div class="text-[13px] font-semibold text-gold sm:text-[15px]">${formatNumber(batch.amount)} ${batch.currency}</div><div class="mt-1 flex justify-end"><span class="rounded px-2.5 py-1 text-[10px] font-semibold tracking-[0.12em] sm:text-[12px] ${badgeClasses(batch.status)}">${batch.status}</span></div><div class="mt-3 flex justify-end"><button data-action="open-batch" data-batch-id="${batch.id}" class="btn-action-flat btn-action-sm inline-flex items-center gap-1.5 text-[11px] font-semibold sm:text-[13px]">${icon('eye','w-4 h-4')} View Details</button></div></div></div><p class="mt-2 text-sm text-zinc-300 sm:text-[15px]">${batch.purpose}</p><p class="mt-1 text-[11px] tracking-[0.12em] text-zinc-500 sm:text-[13px]">Donated: ${batch.date}</p></article>`).join('');
...
function buildCardList() {
  ...
  document.getElementById('main-slot').innerHTML = `<div class="animate-fadeUp"><div class="bg-panel/80 px-6 pt-5 pb-3 backdrop-blur-md"><div class="mx-auto flex max-w-sm rounded-lg border border-line bg-black/30 p-1"><button data-action="card-subtab" data-subtab="egift" class="flex-1 rounded-md py-2 text-[12px] font-semibold uppercase tracking-[0.12em] sm:text-[14px] ${state.cardSubTab === 'egift' ? 'border border-zinc-700 bg-panel text-gold' : 'text-soft'}">E-Gift Card</button><button data-action="card-subtab" data-subtab="redeem" class="flex-1 rounded-md py-2 text-[12px] font-semibold uppercase tracking-[0.12em] sm:text-[14px] ${state.cardSubTab === 'redeem' ? 'border border-zinc-700 bg-panel text-gold' : 'text-soft'}">Redeem History</button></div></div><div class="space-y-5 p-4 sm:p-6">${state.cardSubTab === 'egift' ? `<div class="mb-2 flex items-center justify-between"><div class="relative"><select id="card-filter" class="appearance-none rounded-md border border-line bg-black/30 py-2 pl-3 pr-8 text-[11px] font-semibold uppercase tracking-[0.12em] text-cream outline-none focus:border-gold sm:text-[13px]"><option value="ALL">Filter Status: All</option><option value="Available">Available</option><option value="Gifted">Gifted</option></select><span class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-soft">${icon('chevron-down','w-4 h-4')}</span></div><button data-action="new-donation" data-type="card" class="btn-action inline-flex items-center gap-2 rounded-md px-5 py-2 text-[11px] font-extrabold uppercase tracking-[0.12em] text-black transition hover:scale-105 sm:text-[13px]">${icon('plus','w-4 h-4')} Buy Gift Card</button></div>` : ''}${cards.map(card => `<article class="rounded-2xl border border-line bg-panel p-5 shadow-luxury transition hover:-translate-y-1 hover:border-gold/60"><div class="flex items-start justify-between gap-3"><span class="text-[13px] font-semibold tracking-wide text-cream sm:text-[15px]">CARD ${card.maskedNumber}</span><div class="text-right"><div class="text-[13px] font-semibold text-gold sm:text-[15px]">${formatNumber(card.value)} ${card.currency}</div><div class="mt-1 flex justify-end"><span class="rounded px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] sm:text-[12px] ${badgeClasses(card.status)}">${card.status}</span></div><div class="mt-3 flex justify-end"><button data-action="open-card" data-card-id="${card.id}" class="btn-action-flat btn-action-sm inline-flex items-center gap-1.5 text-[11px] font-semibold sm:text-[13px]">${icon('eye','w-4 h-4')} View Details</button></div></div></div>${card.status === 'Gifted' ? `<p class="mt-2 text-[12px] text-zinc-300 sm:text-[14px] truncate">Recipient: ${card.recipientName}</p>` : ''}${card.status === 'Redeemed' ? `<p class="mt-2 text-[12px] text-zinc-300 sm:text-[14px] truncate">Redeemed by: ${card.redeemerName}</p>` : ''}<p class="mt-1 text-[11px] uppercase tracking-[0.12em] text-zinc-500 sm:text-[13px]">Created: ${card.createdDate}</p></article>`).join('')}</div></div>`;
...
function buildQrView() {
  ...
  document.getElementById('main-slot').innerHTML = fillTemplate('tpl-qr-view', {
    ...
    backIcon: icon('arrow-left','w-4 h-4')
  });
  safeCreateIcons();
}

function buildProofView() {
  document.getElementById('main-slot').innerHTML = fillTemplate('tpl-proof-view');
}
